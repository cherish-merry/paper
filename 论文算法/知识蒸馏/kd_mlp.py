...
def produce_soft_labels(data, round_num, model_name, T, k=0.5):
    ...
    for i in range(round_num):
        ...
        if model_name == "mlp":
            ...
            pred_prob = clf.predict_proba(train_X, T)
...
def evaluate(model_name, k, frac, min_sample_leaf, max_depth, T):
    target_names, data_train, data_eval = load_data(frac=frac)
    soft_label = produce_soft_labels(data_train, 2, model_name, T, k)
...
def export_cm(y_true, y_pred):
    ...
    plt.savefig("confusion_matrix_mlp.png", dpi=500, format="png")
    # plt.show()
...
if __name__ == "__main__":
    ...
    max_depths = [None]
    parameter_space = list(itertools.product(model_names, ks, fracs, min_sample_leafs, max_depths, T))
