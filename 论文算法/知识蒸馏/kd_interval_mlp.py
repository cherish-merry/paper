...
def produce_soft_labels(data, round_num, k=0.5):
...
def evaluate(k, frac, min_sample_leaf, max_depth):
    target_names, data_train, data_eval = load_data(frac=frac)
    soft_label = produce_soft_labels(data_train, 1, k)
    # 学生
...
if __name__ == "__main__":
    ...
    # 决策树最大深度
    max_depths = [None]
    parameter_space = list(itertools.product(ks, fracs, min_sample_leafs, max_depths))
